...
namespace Rotation_Checking_Algorithm
{
    internal class Program
    {
        ...
        static double NCC(Mat a, Mat b, Mat mask)
        {
            Mat af = new();
            Mat bf = new();
            a.ConvertTo(af, MatType.CV32F); // Chuyển ảnh ra dạng số thực cho các giá trị pixel để lát nhân chia cộng trừ không bị mất số thập phân, thêm chính xác
        ...
        static double BruteForce(Mat refgray, Mat testgray) // Thuật toán Brute Force Matching để so sánh 2 bức ảnh với nhau
        {
            ...
            var ct = FindCircle(refgray)!.Value; // Dấu chấm than là tôi cam kết là kết quả không phải null đừng cảnh báo tui
        ...
        static void Main(string[] args)
        {
            ...
            Mat imgtest = Cv2.ImRead(@"C:\Users\fs120806\Desktop\Document\Programing Project\Net_Programming\Image Library\test\base_A_003.jpg", ImreadModes.Grayscale);
            // Nhấn ctrl với click chuột trái để xem hàm khai báo hoặc ctrl với Q để xem quick docummentation
            // Trong OpenCV thì trục Y hướng xuống nha
            Mat mask = Mat.Zeros(A.Size(), MatType.CV_8UC1);
            Cv2.Circle(mask, new Point(r, r), r - 6, Scalar.All(255), -1);
